...
export default function Mode(){
    const { theme, setTheme } = useContext(SetContext)
    const { setCalendarThemeBool } = useContext(SetContext);
    const spanRef = useRef()
    const labelRef = useRef()

    const handleNightMode = (e) => {
        setTheme(theme => !theme);
        setCalendarThemeBool(theme => !theme);
        e.preventDefault();
    ...
    useEffect(()=> {
        if(localStorage.theme == "dark"){
            spanRef.current.classList.replace("translate-x-1", "!translate-x-[119%]");
            labelRef.current.classList.replace("bg-slate-300", "bg-blue-600"); 
        }else{
            spanRef.current.classList.replace("!translate-x-[119%]", "translate-x-1");
            labelRef.current.classList.replace("bg-blue-600", "bg-slate-300");   
        }
    }, [theme])

    return (
        <li onClick={handleNightMode}>
            <label className="relative m-0 block h-7 w-14 rounded-full bg-slate-300 cursor-pointer modeLabel" ref={labelRef}>
                <span className="absolute top-1/2 flex h-6 w-6 -translate-y-1/2 translate-x-1 items-center justify-center rounded-full bg-white duration-100 ease-linear modeSpan" ref={spanRef}>
                    {/* Light Mode */}
                    <span className="dark:hidden" id="light">
